...
export const useNewChatConversation = ({
  ...
  // Looking up "the conversation with these people" only makes sense when the
  // existing room matches what is being asked for. The driver checks this too;
  // skipping the lookup here avoids showing the wrong room while it resolves.
  const { chat } = useChatForUsers(
    isNew && !willEncrypt ? selectedUserIds : [],
  );
