...
def fixture_albert_calls(monkeypatch, inline):  # pylint: disable=unused-argument
...
def _chat(client, meeting, *messages):
    return client.post(
...
def test_api_scribe_chat_ping_gets_an_answer(albert_calls, scribe_client):
    ...
    assert (
        _chat(
            scribe_client, meeting, _message("m1", "@ariane résume la réunion")
        ).status_code
        == HTTP_204_NO_CONTENT
    )
    ...
    response = _replies(scribe_client, meeting)
    ...
    # Taken once.
    assert _replies(scribe_client, meeting).json()["replies"] == []


@override_settings(**SETTINGS)
def test_api_scribe_chat_without_ping_is_only_kept(albert_calls, scribe_client):
    ...
    _chat(scribe_client, meeting, _message("m3", "@ariane bonjour"))
    _chat(scribe_client, meeting, _message("m3", "@ariane bonjour"))
...
def test_api_scribe_chat_without_albert(albert_calls, scribe_client):
...
@override_settings(**SETTINGS)
def test_api_scribe_chat_help_is_answered_without_albert(albert_calls, scribe_client):
    """The help is a canned answer."""
    meeting = factories.MeetingFactory()

    _chat(scribe_client, meeting, _message("m1", "@ariane /aide"))
...
def test_api_scribe_chat_albert_failure(monkeypatch, scribe_client):
...
@override_settings(**SETTINGS)
def test_meeting_assistant_reads_only_since_the_asker_arrived(
    albert_calls, scribe_client
):
    ...
    _chat(scribe_client, meeting, _message("m1", "@ariane de quoi a-t-on parlé ?"))
...
@override_settings(**SETTINGS)
def test_meeting_assistant_unknown_asker_reads_nothing_before(
    albert_calls, scribe_client
):
    ...
    _chat(scribe_client, meeting, _message("m1", "@ariane résume", identity="inconnu"))
...
@override_settings(**SETTINGS)
def test_api_scribe_replies_closed_meeting(scribe_client):
    """A closed meeting has nothing more to post."""
    meeting = factories.MeetingFactory(closed_at=timezone.now())

    assert _replies(scribe_client, meeting).status_code == HTTP_410_GONE
