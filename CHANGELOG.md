...
- ✨(bots) Add Ariane, an assistant reachable with `@Ariane` in any room she
  has been invited to.
  She answers only when addressed, reads the thread and the room history that
  followed her arrival, and changes register with `/juriste`, `/avocat`, `/po`
  and `/pm`. Answers come from the Albert API, from the server only. The
  context is cut at both her own arrival and the asker's history horizon, so
  she never reads back what either of them is not entitled to. She cannot work
  in encrypted rooms, which includes every private message. `/aide` lists the
