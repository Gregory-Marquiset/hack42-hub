...
import { useResolvedAvatarUrl } from "./useAvatarSrc";

/**
 * The signed-in account's own avatar, resolved to a URL an `<img>` can load
 * (see `useResolvedAvatarUrl`). Fetched from the driver on mount — not just
 * right after a change via `useSetUserAvatar` — so a photo set in a previous
...
export const useMyAvatarSrc = (accountId: AccountId): string | undefined => {
  ...
  return useResolvedAvatarUrl(accountId, url);
};
